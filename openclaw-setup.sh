...
用法: openclaw-setup {install|config|repair|workbench|status|doctor|engine|migrate|skills|backup|help}
...
case "$cmd" in
  ...
  skills)
    ...
    case "$action" in
      ...
      enrich)
        shift || true
        enrich_all_skills_metadata "${1:-}"
        ;;
      update)
        shift || true
        update_all_skills_from_source "${1:-}"
        ;;
      show-source|source)
        shift || true
        skill_name="${1:-}"
        if [ -z "$skill_name" ]; then
          echo "用法: openclaw-setup skills show-source <skill_name>" >&2
          exit 1
        fi
        shift || true
        show_skill_source "$skill_name" "${1:-}"
        ;;
      *)
        RULE_PROFILE_SELECTED="$action" sync_skills "$action"
        ;;
