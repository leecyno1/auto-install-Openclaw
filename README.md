...
已按最新官方安装器/文档逐项核对并更新，重点包括：

- 安装参数与环境变量语义对齐（`--install-method`、`--no-onboard`、`--dry-run`、`OPENCLAW_*`）。
- Core 安装链路委托官方 `https://openclaw.ai/install.sh`，减少行为漂移。
- Node 版本门槛对齐到 `22.12+`。
- 飞书插件固定为官方包，并在配置前做同名插件清理再重装。
- 飞书凭据写入结构对齐官方 `channels.feishu.accounts.main.*`。
- 升级链路统一为 `update -> doctor -> plugins update --all`。

详细检查清单见：`docs/official-compatibility-checklist.md`
