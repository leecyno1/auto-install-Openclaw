...
class ConfigSurfaceTests(unittest.TestCase):
    ...
    def test_cli_surfaces_use_dasheng_red_blue_theme(self):
        ...
        self.assertIn('引擎配置中心', menu_text)
        self.assertIn('仅显示当前机器实际安装/启用的引擎状态', menu_text)
        self.assertNotIn('OpenClaw / Hermes 双轨     [ ${THEME_OK}OK${NC} ]', menu_text)
        self.assertNotIn('红蓝主题终端面板', menu_text)
        self.assertIn('[ ${THEME_OK}OK${NC} ]', install_text)
