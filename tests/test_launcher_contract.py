...
class LauncherContractTests(unittest.TestCase):
    ...
    def test_install_script_wires_health_and_quota_helpers_into_pixel_house_startup(self):
        install_text = (ROOT / 'install.sh').read_text(encoding='utf-8')
        ...
        self.assertIn('run_as_root systemctl enable "$LOBSTER_WORLD_SERVICE_NAME" "$LOBSTER_PROJECTION_SERVICE_NAME" "$LOBSTER_BRIDGE_SERVICE_NAME" "$LOBSTER_QUOTA_SERVICE_NAME" "$LOBSTER_HEALTH_SERVICE_NAME"', install_text)
        self.assertIn('run_as_root systemctl restart "$LOBSTER_QUOTA_SERVICE_NAME"', install_text)
        self.assertIn('run_as_root systemctl restart "$LOBSTER_HEALTH_SERVICE_NAME"', install_text)
        self.assertIn('nohup python3 "$CONFIG_DIR/scripts/gateway-quota-enforcer.py" restart >/tmp/openclaw-quota-enforcer.log 2>&1 &', install_text)
