import { type HomeStationId, type RuntimeStateId, type TaskPlan, type TaskStep } from '../../../shared/world-model.js';

function detailFor(state: RuntimeStateId, prompt: string): string {
  switch (state) {
    case 'researching':
...
export function createTaskPlan(input: { prompt: string; stationId: HomeStationId }): TaskPlan {
...
export function stepTaskPlan(plan: TaskPlan, index: number): TaskStep {
