...
import { applyBuildToOpenClaw, dispatchTaskToConnector, readOpenClawRuntime, } from "./connectors/openclaw.js";
...
function reserveTextQuota(units = 1) {
    const script = resolveQuotaScript();
    if (!script) {
        return { ok: false, reason: "quota_script_missing", message: "quota script not found" };
    }
    const result = spawnSync("python3", [script, "reserve", "--category", "text", "--units", String(Math.max(1, units)), "--tool", "openclaw-world-gateway"], { encoding: "utf8", env: process.env });
...
function commitQuotaReservation(reservationId) {
    if (!reservationId)
        return;
    const script = resolveQuotaScript();
    if (!script)
...
function releaseQuotaReservation(reservationId) {
    if (!reservationId)
        return;
    const script = resolveQuotaScript();
    if (!script)
