...
const openclawStatusUrl = process.env.OPENCLAW_STATUS_URL || "http://127.0.0.1:13145/status";
const openclawSkillsDir = path.join(openclawHome, "skills");
const managedSkillsFile = path.join(openclawHome, "profile", "world-managed-skills.json");
const worldBuildMirror = path.join(openclawHome, "profile", "openclaw-world-build.json");
...
function runConfigSet(key, value) {
    spawn("bash", ["-lc", `openclaw config set ${JSON.stringify(key)} ${JSON.stringify(value)}`], {
        detached: true,
        stdio: "ignore",
    }).unref();
}
function safeSkillIdList(input) {
    return [...new Set(input.map((item) => String(item || "").trim()).filter(Boolean))];
}
function listInstalledSkillDirs() {
    try {
        return fs
...
function syncManagedSkills(desiredSkills) {
    fs.mkdirSync(openclawSkillsDir, { recursive: true });
    const managed = readJsonFile(managedSkillsFile, {
        managedSkills: [],
    });
...
export async function readOpenClawRuntime(previous) {
    try {
        const response = await fetch(openclawStatusUrl, { cache: "no-store" });
        if (!response.ok)
            throw new Error(`HTTP ${response.status}`);
        const payload = (await response.json());
        const stateText = String(payload.state || payload.status || payload.phase || previous.state).toLowerCase();
        ...
        const normalizedState = (["idle", "researching", "executing", "syncing", "error"].includes(stateText)
            ? stateText
            : previous.state);
        return {
            reachable: true,
...
export function applyBuildToOpenClaw(build) {
...
export function dispatchTaskToConnector(taskPayload) {
...
export function readInstalledSkillIds() {
